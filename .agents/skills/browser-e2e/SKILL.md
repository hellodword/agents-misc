...
6. When project configuration does not select a display mode, run headless. Run headful only when the user explicitly requests it; then require a supported browser and Linux display, fail clearly when either is absent, and never download a browser or silently switch modes.
