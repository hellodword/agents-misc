...
| Nix, Just, flake outputs, or command workflow changes                      | `.agents/rules/toolchain/nix-just.md`; `.agents/rules/toolchain/flake-organization.md` when flake outputs change; `.agents/rules/core/scripts.md` when scripts change | `.agents/skills/nix-just-workflow/SKILL.md`                                                          | `.agents/references/nixpkgs-devcontainer-alignment.md`; `.agents/templates/treefmt.nix`, `.agents/templates/.prettierrc.json`, and `.agents/templates/.editorconfig` when seeding treefmt-based formatting |
