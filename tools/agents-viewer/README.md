...
Desktop uses resizable session and conversation panels; the inspector starts collapsed and opens only when requested. Tablet moves the inspector into a sheet; mobile moves both side panels into sheets. The session panel groups conversations by `parentThreadId`, orders groups by their newest activity, and renders always-expanded parent/child trees with source avatars, friendly update times, previews, and working-directory subtitles. Explicit parent metadata, subagent spawn metadata, and `forked_from_id` all use the same tree. A fresh-context plan implementation is linked only when its fixed Codex handoff prefix, normalized plan SHA-256, non-empty working directory, and ordering exactly match a preceding `<proposed_plan>`; the child is labeled “Implement · parent title”. There is no title/time/similarity fallback, and a missing indexed parent leaves the child as a browsable root. User messages appear as right-aligned chat bubbles and assistant replies as left-aligned bubbles. Messages use sanitized GFM rendering, local clock times, human-readable day separators, and a copy control that retrieves the complete message on demand.

Reasoning and commands appear as compact left-aligned activity text beginning with `Reasoning:` and `Executing:`; activity rows do not repeat timestamps. Multiline activity shows only its first line with an ellipsis, while hover or keyboard focus reveals the complete text. Command output is never shown in the conversation; click the activity to inspect its complete input, result, and raw records. Warnings and errors remain visible. Turn/world context, turn markers, token usage, patches, `wait`, plans, other tools, and injected instructions are hidden by default. The top-bar Filter dialog controls source, working directory, archive state, and “Show technical activity”; its technical-activity choice is stored locally in the browser.
...
Global search defaults to active User and Assistant conversation messages. “Search all activity types” additionally searches reasoning, commands and results, context, and other technical entries; this choice is stored in the browser. `GET /api/v1/search` exposes the same scope as the optional `allTypes=true` query parameter, which defaults to `false`. The broad scope uses bounded fallback scanning for text-bearing entries that are deliberately absent from FTS and reports `partial=true` if that bound is reached. The sidebar uses `GET /api/v1/session-groups`; pagination counts whole trees, and a source/cwd/archive filter that matches any member returns that complete tree. The original flat `GET /api/v1/sessions` contract remains available.
...
| thread parent/spawn/fork metadata                 | `parentThreadId` plus `parentRelation`                                | explicit parent wins over fork; unresolved parents remain orphans      |
| fresh-context plan implementation                 | derived `parentThreadId` with `parentRelation=planHandoff`            | exact prefix/plan hash/cwd/order match only; no fuzzy inference         |
