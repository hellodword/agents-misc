...
import type { EntryListItem, SessionGroup, SessionSummary } from "@/generated/api"
...
const sessionGroups:SessionGroup[]=[{root:{session,children:[]},latestSessionId:"s1",updatedAt:session.updatedAt}]
...
beforeEach(()=>{
  ...
  vi.stubGlobal("fetch",vi.fn(async(input:string|URL|Request)=>{
    ...
    else if(url.includes("/session-groups"))body={data:sessionGroups,partial:false}
...
describe("Agents Viewer UI",()=>{
  ...
  it("does not request session filters until Apply",async()=>{const user=userEvent.setup();render(<MemoryRouter initialEntries={["/sessions/s1"]}><App/></MemoryRouter>);expect(await screen.findByRole("heading",{name:"Hello session"})).toBeInTheDocument();const before=callsFor("/api/v1/session-groups?");await user.click(screen.getByRole("button",{name:"Filter"}));await user.type(screen.getByRole("textbox",{name:"Working directory"}),"/work/demo");expect(callsFor("/api/v1/session-groups?")).toBe(before);await user.click(screen.getByRole("button",{name:"Cancel"}));await user.click(screen.getByRole("button",{name:"Filter"}));await user.type(screen.getByRole("textbox",{name:"Working directory"}),"/work/demo");await user.click(screen.getByRole("button",{name:"Apply"}));await waitFor(()=>expect(callsFor("/api/v1/session-groups?")).toBe(before+1));expect(vi.mocked(fetch).mock.calls.some(([input])=>String(input).includes("cwd=%2Fwork%2Fdemo"))).toBe(true)})
  it("renders every parentThreadId relationship as an expanded tree",async()=>{const parent={...session,title:"Plan session"};const child:SessionSummary={...session,id:"s2",title:"Review child",parentThreadId:"s1",parentRelation:"parent",source:"subagent"};const handoff:SessionSummary={...session,id:"s3",title:"Handoff payload",parentThreadId:"s1",parentRelation:"planHandoff",source:"exec"};const groups:SessionGroup[]=[{root:{session:parent,children:[{session:child,children:[]},{session:handoff,children:[]}]},latestSessionId:"s3",updatedAt:handoff.updatedAt}];const fallback=vi.mocked(fetch);vi.stubGlobal("fetch",vi.fn((input:string|URL|Request,init?:RequestInit)=>String(input).includes("/session-groups")?Promise.resolve(new Response(JSON.stringify({data:groups,partial:false}),{status:200,headers:{"content-type":"application/json"}})):fallback(input,init)));const {container}=render(<MemoryRouter initialEntries={["/sessions/s1"]}><App/></MemoryRouter>);expect(await screen.findByText("Review child")).toBeInTheDocument();expect(screen.getByText("Implement · Plan session")).toBeInTheDocument();expect(container.querySelectorAll(".session-children > .session-tree-node")).toHaveLength(2);expect(container.querySelector('a[href="/sessions/s1"]')).toHaveAttribute("aria-current","page")})
  it("renders copyable chat bubbles and single-line activity without per-item times",async()=>{const user=userEvent.setup();const writeText=vi.spyOn(navigator.clipboard,"writeText");const inspect=vi.fn();const reasoning={...entry,id:"e2",sequence:2,kind:"reasoning" as const,presentation:"technical" as const,role:undefined,title:"Reasoning",primaryPreview:"First useful line",defaultCollapsed:true};const command={...entry,id:"e3",sequence:3,kind:"tool" as const,presentation:"technical" as const,role:undefined,toolKind:"command" as const,title:"exec_command",primaryPreview:'{"cmd":"printf hello\\nprintf world"}',secondaryPreview:"secret output",defaultCollapsed:true};const {container}=render(<VirtualTranscript entries={[entry,reasoning,command]} onInspect={inspect}/>);expect(container.querySelector(".message-user")).toBeInTheDocument();expect(screen.getByText("Reasoning:")).toHaveClass("activity-label");expect(screen.getByText("First useful line")).toHaveClass("activity-body");expect(screen.getByText("Executing:")).toHaveClass("activity-label");const executingButton=screen.getByRole("button",{name:/Executing: printf hello/});expect(executingButton.querySelector(".activity-body")).toHaveTextContent("printf hello…");await user.hover(executingButton);expect(await screen.findByRole("tooltip")).toHaveTextContent(/printf hello\s+printf world/);expect(screen.queryByText("secret output")).not.toBeInTheDocument();const reasoningButton=screen.getByRole("button",{name:/Reasoning: First useful line/});expect(within(reasoningButton).queryByRole("time")).not.toBeInTheDocument();await user.click(reasoningButton);expect(inspect).toHaveBeenCalledWith("e2");await user.click(screen.getByRole("button",{name:"Copy message"}));expect(writeText).toHaveBeenCalledWith("Hello **world**");expect(executedContent('{"action":{"command":["git","status"]}}')).toBe("git status");expect(isDefaultVisible(command)).toBe(true)})
  ...
  it("uses one event stream and coalesces refreshes by event type",async()=>{
    ...
    await waitFor(()=>expect(callsFor("/api/v1/session-groups?")).toBe(listBefore+1))
